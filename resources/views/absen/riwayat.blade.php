        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-3xl font-bold text-white mb-2">
                    📅 Riwayat Absen <span class="text-yellow-400">{{ \Carbon\Carbon::parse($bulan)->translatedFormat('F Y') }}</span>
                </h2>
                <p class="text-gray-400 text-lg">Pantau kehadiran Anda dengan tampilan kalender yang interaktif</p>
            </div>
            <div class="hidden md:block">
                <div class="w-24 h-24 rounded-full bg-gradient-to-br from-yellow-400/20 to-amber-500/20 flex items-center justify-center border border-yellow-400/30">
        ...
        <div class="bg-gradient-to-br from-green-500/10 to-green-600/10 border border-green-500/20 rounded-xl p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-green-300 text-xs">{{ number_format(($statusCounts['H'] / $totalDays) * 100, 1) }}% dari bulan ini</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-yellow-500/10 to-amber-500/10 border border-yellow-500/20 rounded-xl p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-yellow-300 text-xs">{{ number_format(($statusCounts['T'] / $totalDays) * 100, 1) }}% dari bulan ini</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-red-500/10 to-red-600/10 border border-red-500/20 rounded-xl p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-red-300 text-xs">{{ number_format(($statusCounts['A'] / $totalDays) * 100, 1) }}% dari bulan ini</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-blue-500/10 to-blue-600/10 border border-blue-500/20 rounded-xl p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-blue-300 text-xs">{{ number_format(($statusCounts['I'] / $totalDays) * 100, 1) }}% dari bulan ini</p>
                </div>
        ...
        <div class="grid grid-cols-7 gap-2">
            ...
            @while($currentDate <= $endOfCalendar)
                ...
                <div class="relative group">
                    <div class="aspect-square p-2 rounded-lg border transition-all duration-300 hover:scale-105 cursor-pointer
                        ...
                        @else border-gray-600 bg-gray-800/50
                        @endif
                        @if($absenItem)
                            @if($absenItem->status == 'H') border-green-500 bg-green-500/20
                            ...
                            @elseif(in_array($absenItem->status, ['O', 'C'])) border-purple-500 bg-purple-500/20
                            @endif
                        @endif">
                        ...
                        <!-- Tooltip -->
                        @if($absenItem && $isCurrentMonth)
                            <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10 whitespace-nowrap">
                                @php
                                    $statusLabels = [
                                        'H' => 'Hadir',
                                        'T' => 'Terlambat',
    ...
    <div class="glass-card rounded-2xl p-8">
                ...
                <table class="w-full">
                    <thead class="bg-gray-800/50">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Keterangan</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-gray-800/30 transition-colors duration-200">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            <div class="text-sm font-medium text-white">{{ $a->tanggal->format('d F Y') }}</div>
                                            <div class="text-sm text-gray-400">{{ $a->tanggal->translatedFormat('l') }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                        $statusConfig = [
                                            ...
                                            'S' => ['color' => 'blue', 'label' => 'Sakit', 'icon' => 'user-injured'],
                                            'O' => ['color' => 'purple', 'label' => 'Off', 'icon' => 'home'],
                                            'C' => ['color' => 'purple', 'label' => 'Cuti', 'icon' => 'umbrella-beach'],
                                        ];
                                        $config = $statusConfig[$a->status] ?? ['color' => 'gray', 'label' => $a->status, 'icon' => 'question'];
                                    @endphp
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-{{ $config['color'] }}-500/20 text-{{ $config['color'] }}-400 border border-{{ $config['color'] }}-500/30">
        ...
        @else
            <div class="text-center py-12">
                <div class="p-4 bg-gradient-to-r from-yellow-400 to-amber-500 rounded-full w-16 h-16 mx-auto mb-4 flex items-center justify-center">
                ...
                <p class="text-gray-400">Belum ada data kehadiran untuk bulan {{ \Carbon\Carbon::parse($bulan)->translatedFormat('F Y') }}</p>
            </div>
        @endif
    </div>
</div>
