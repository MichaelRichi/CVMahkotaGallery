...
<div class="space-y-6">
    ...
    <div class="glass-card rounded-2xl p-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <button id="processPayroll" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-green-500 to-green-600 text-white font-semibold rounded-xl hover:from-green-600 hover:to-green-700 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-green-400/25">
                    ...
                    @forelse($staff as $s)
                        <tr class="hover:bg-gray-800/30 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 text-right">
                                <span class="text-white font-medium">Rp {{ number_format($s->gaji_pokok, 0, ',', '.') }}</span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <span class="text-white font-medium">Rp {{ number_format($s->gaji_tunjangan, 0, ',', '.') }}</span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <span class="text-red-400 font-medium">Rp {{ number_format($s->potongan_kronologi, 0, ',', '.') }}</span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <span class="text-red-400 font-medium">Rp {{ number_format($s->potongan_peminjaman, 0, ',', '.') }}</span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <span class="text-white font-medium">Rp {{ number_format($s->gaji_bersih, 0, ',', '.') }}</span>
                            </td>
                        </tr>
                    @empty
                        <tr>
...
{{-- <script>
    document.getElementById('processPayroll').addEventListener('click', function() {
        if (confirm('Apakah Anda yakin ingin memproses penggajian untuk semua staff pada {{ now()->format('F Y') }}?')) {
            window.location.href = '{{ route('admin.salary.process') }}';
        }
    });
</script> --}}
@endpush
